        ...
        for page_no, page in enumerate(doc):
            ...
            for i, t in enumerate(temp_page):
                if t[-1] == 0:
                    temp = {}
                    temp[page_no] = [i]
                    current = sent_tokenize(t[4].strip().replace("\r\n", " ").replace("\n", " "))
                    current = " ".join(current)
                    
                    if len(paras) > 0 and prev != "" and (
                        not para_is_complete(paras[-1][0]) or 
                        prev_n_words < utils.ATTACH_N_WORD_THRESHOLD
                    ):
                        attach = True
                    else:
                        attach = False
...
def create_train_df(elements, id_col):
    count = 0
    df = pd.DataFrame(index=range(len(elements)), columns=[id_col, "passage", "para", "filename", "page", "display", "highlight"])
    for i, elem in enumerate(elements):
        sents = sent_tokenize(elem[2])
        sents = list(map(lambda x: x.lower(), sents))
        passage = " ".join(sents)
        # elem[-1] is id
        df.iloc[i] = [elem[-1], passage, passage, elem[4], elem[3], elem[2], elem[5]]
        count = count + 1

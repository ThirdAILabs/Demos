...
class Model:
    ...
    def infer_samples_to_infer_batch(self, samples: InferSamples):
        query_col = self.get_query_col()
        return [
            {query_col: utils.clean_text(text)} for text in samples
        ]
    
    def train_buckets(self, samples: TrainSamples, learning_rate: float, **kwargs) -> None:
        raise NotImplementedError()

    def infer_buckets(self, samples: InferSamples, n_results: int, **kwargs) -> Predictions:
        raise NotImplementedError()
    
    def train_labels(self, samples: TrainSamples, learning_rate: float, **kwargs) -> None:
        raise NotImplementedError()

    def infer_labels(self, samples: InferSamples, n_results: int, **kwargs) -> Predictions:
        raise NotImplementedError()
...
def unsupervised_train_on_docs(
    ...
    for i in range(max_epochs):
        ...
        on_progress(accuracy=val)
        if i>=min_epochs-1 and val > acc_to_stop:
            break


def make_balancing_samples(references: pd.Series):
    references = references.sample(min(25000, len(references)))
    samples = [(query, [i]) for i, passage in references.items() for query in sent_tokenize(passage)]
...
class Standard(Model):
    def __init__(self, id_col="DOC_ID", id_delimiter=" ", query_col="QUERY", input_dim=100_000, hidden_dim=512):
        self.id_col = id_col
    ...
    def index_documents(
        ...
        self.balancing_samples = make_balancing_samples(documents.get_new_references())
    ...
    def train_labels(self, samples: TrainSamples, learning_rate: float, **kwargs) -> None:
        self.model.train_batch(self.train_samples_to_train_batch(samples), learning_rate=learning_rate)
    ...
    def balance_train_label_samples(self, samples: TrainSamples, n_samples: int):
        balanced_samples = samples + list(random.choices(self.balancing_samples, k=n_samples))
...
class Mach(Model):
    def __init__(self, id_col="DOC_ID", id_delimiter=" ", query_col="QUERY", input_dim=50_000, hidden_dim=2048, extreme_output_dim=50_000):
        self.id_col = id_col
        self.id_delimiter = id_delimiter
        self.query_col = query_col
        self.input_dim = input_dim
    ...
    def index_documents(
        ...
        if config.id_col != self.id_col:
            raise ValueError(f"Model configured to use id_col={self.id_col}, received document with id_col={config.id_col}")
        ...
        if self.model is None:
            ...
            if config.n_new_ids > 0:
                ...
                if doc_id != self.id_col:
                    raise ValueError(f"Document has a different id column ({doc_id}) than the model configuration ({self.id_col}).")
    ...
    def add_balancing_samples(self, documents: Documents):
        samples = make_balancing_samples(documents.get_new_references())
    ...
    def infer_labels(self, samples: InferSamples, n_results: int, **kwargs) -> Predictions:
        ...
        #####
        return [[int(pred) for pred, _ in predictions] for predictions in all_predictions]
    ...
    def infer_buckets(self, samples: InferSamples, n_results: int, **kwargs) -> Predictions:
        infer_batch = self.infer_samples_to_infer_batch(samples)
        predictions = [
            self.model.predict_hashes(sample)[:n_results] 
            for sample in infer_batch
        ]
        return predictions
